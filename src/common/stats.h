...
struct Stats {
 public:
  ...
  void dump() {
    ...
    std::cout << "    Num total write: " << _n_write_dup_content + _n_write_not_dup << " times" << std::endl
    ...
    std::cout << "    Num total read: " << _n_read_hit + _n_read_not_hit << " times" << std::endl
    ...
    std::cout << "IO statistics: " << std::endl
              ...
              << "    Num bytes metadata written to pm: " << _n_metadata_bytes_written_to_pm << std::endl
              << "    Num bytes metadata read from pm: " << _n_metadata_bytes_read_from_pm << std::endl
              << "    Num bytes data written to pm: " << _n_data_bytes_written_to_pm << std::endl
              << "    Num bytes data read from pm: " << _n_data_bytes_read_from_pm << std::endl
              << std::endl;
    ...
    std::cout << std::fixed << std::setprecision(0) << "Time Elapsed: " << std::endl
              ...
              << "    Time elpased for io_pm: " << _time_elapsed_io_pm << std::endl
              << "    Time elpased for update_index(FPBucket_update): " << _time_elapsed_FPBucket_update << std::endl
              << "    Time elpased for update_index(computeCachedataLocation): "
              << _time_elapsed_computeCachedataLocation << std::endl
              << "    Time elpased for update_index(computeMetadataLocation): " << _time_elapsed_computeMetadataLocation
              << std::endl
    ...
    std::cout << std::setprecision(2) << "Overall Stats: " << std::endl
              ...
              << "    Dup ratio: "
              << 1.0 * (_n_write_dup_content + _n_read_not_hit_dup_content) / (_n_write + _n_read_not_hit) * 100.0
              << "%" << std::endl
              << "    Dup ratio (not include read): " << 1.0 * _n_write_dup_content / _n_write * 100.0 << "%"
              << std::endl;
  ...
  inline void setCurrentRequestType(bool is_write) {
    if (is_write)
      _current_request_type = 1;
    else
      _current_request_type = 0;
  }
  ...
  inline void add_read_post_dedup_stat(Chunk &c) {
    if (c.dedupResult_ == DUP_CONTENT) {
      ...
      if (c.hitFPIndex_ == false) {
        ...
        if (c.verficationResult_ == BOTH_LBA_AND_FP_NOT_VALID || c.verficationResult_ == ONLY_LBA_VALID) {
  ...
  inline int get_current_request_type() { return _current_request_type; }
  /*
...
#define _(str)                  \
  ...
  inline void add_time_elapsed_##str(uint64_t v) { _time_elapsed_##str += v; }
...
#undef _
  ...
  std::atomic<uint64_t> _n_data_bytes_written_to_pm;
  std::atomic<uint64_t> _n_data_bytes_read_from_pm;
  std::atomic<uint64_t> _n_metadata_bytes_written_to_pm;
  std::atomic<uint64_t> _n_metadata_bytes_read_from_pm;

  inline void add_total_bytes_written_to_ssd(uint64_t v) {
    _n_total_bytes_written_to_ssd.fetch_add(v, std::memory_order_relaxed);
  }
  inline void add_bytes_written_to_ssd(uint64_t v) {
    _n_data_bytes_written_to_ssd.fetch_add(v, std::memory_order_relaxed);
  }
  inline void add_bytes_read_from_ssd(uint64_t v) {
    _n_data_bytes_read_from_ssd.fetch_add(v, std::memory_order_relaxed);
  }

  inline void add_metadata_bytes_written_to_ssd(uint64_t v) {
    _n_metadata_bytes_written_to_ssd.fetch_add(v, std::memory_order_relaxed);
  }
  inline void add_metadata_bytes_read_from_ssd(uint64_t v) {
    _n_metadata_bytes_read_from_ssd.fetch_add(v, std::memory_order_relaxed);
  }

  inline void add_metadata_bytes_written_to_pm(uint64_t v) {
    _n_metadata_bytes_written_to_pm.fetch_add(v, std::memory_order_relaxed);
  }
  inline void add_metadata_bytes_read_from_pm(uint64_t v) {
    _n_metadata_bytes_read_from_pm.fetch_add(v, std::memory_order_relaxed);
  }
  inline void add_bytes_written_to_pm(uint64_t v) {
    _n_data_bytes_written_to_pm.fetch_add(v, std::memory_order_relaxed);
  }
  inline void add_bytes_read_from_pm(uint64_t v) { _n_data_bytes_read_from_pm.fetch_add(v, std::memory_order_relaxed); }
  ...
  void reset() {
    ...
    _n_metadata_bytes_written_to_pm.store(0, std::memory_order_relaxed);
    _n_metadata_bytes_read_from_pm.store(0, std::memory_order_relaxed);
...
#define _(str) _time_elapsed_##str = 0;
    _(compression);
    _(decompression);
    _(fingerprinting);
    _(dedup);
